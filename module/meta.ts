...
interface MetaData {
	"meta/alias": Record<string, string[]>;
	"meta/cartoon": string[];
}
...
export class MetaManagement {
	...
	public watchStart() {
		...
		eventList.forEach( event => {
			this.watcher!.on( event, async ( filePath: string ) => {
				...
				if ( event !== "add" ) {
					this.logger.info( `[azur-lane]静态数据文件 [${ metaFile }] 内容更新` );
				}
			} )
		} )
	}
	...
	private async loadMeta( metaFile: keyof MetaData ) {
		const data = await this.file.loadYAML( `azur-lane/assets/${ metaFile }`, "plugin" );
